...
async def get_pkg_manager():
    ...
    options = [Option(
        pm["name"], f"{pm['description']} {'- ' + pm['note'] if pm['note'] else ''}") for pm in PACKAGE_MANAGERS]
...
def run(cmd: list, cwd: Path, interactive: bool = False) -> None:
    ...
    try:
        ...
        raise click.ClickException(
            f"Error running command '{' '.join(cmd)}' in path {cwd}: {e}")
...
@click.option("-i", "--immediate", is_flag=True, help="Install dependencies immediately after project creation.")
@click.option("--interactive", is_flag=True, default=False, help="Show detailed setup and installation logs.")
@config
    ...
    try:
        ...
        if target_path.exists() and any(target_path.iterdir()):
            ...
            if res == "cancel" or is_cancel(res):
                ...
                ctx.log(f"Overwritten existing directory '{target_path}'.")
            else:
                ctx.log(f"Merging with existing files in '{target_path}'.")
        # Create project structure based on inputs
        copy_template(pkg_manager, target_path, project_name)
        ctx.log(f"Project '{project_name}' created at {target_path.resolve()}")
        ...
        if immediate:
            async with spinner("Setting up project...") as spin:
                try:
                    ...
                    try:
                        click.secho(
                            "Starting development server...", fg="green")
